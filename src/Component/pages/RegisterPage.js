...
const RegisterPage = (props) => {
  const baseUrl = "http://35.93.77.214:9090/auth/register";
  ...
  const handleChange = (e, type) => {
    if (type === "email") {
      ...
      if (e.target.value.length > 8 && e.target.value.length < 16) {
        ...
        } else if ((e.target.value.length < 8 || e.target.value.length > 16) && e.target.value !== "") {
          setUserNameError({
    ...
    <>
      ...
      <div className="overflow-x-hidden flex py-44 items-center flex-col-reverse justify-center w-full md:gap-x-80  lg:flex-row">
        ...
        <div className="border-slate-200 rounded-xl shadow-md p-12 w-96">
          <form className="text-sm max-w-[460px] h-full sm:max-w-full" onSubmit={handleSubmit}>
            <img src={logo} alt="logo" className="w-32 mx-auto mb-5" />
            <label htmlFor="email">
              <span className="block font-semibold mb-1 after:content-['*'] after:ml-0.5 ">Email</span>
              <input
              ...
              />
              <p className={`mb-0 text-sm ${emailError.status ? "text-green-700" : "text-pink-700"}`}>{emailError.message}</p>
            </label>
            <label htmlFor="text">
              <span className="block mt-4 font-semibold mb-1 after:content-['*'] after:ml-0.5 ">Username</span>
              <input
              ...
              />
              <p className={`mb-0 text-sm ${userNameError.status ? "text-green-700" : "text-pink-700"}`}>{userNameError.message}</p>
            </label>
            <label htmlFor="password">
              <span className="block mt-4 font-semibold mb-1 after:content-['*'] after:ml-0.5 ">Password</span>
              <input
                placeholder="********"
                id="password"
                className=" w-full block border rounded border-orange-400 bg-gray-100 p-2 focus:outline-none focus:ring-orange-500 focus:border-orange-500 "
              ...
              />
              <p className={`mb-0 text-sm ${passwordError.status ? "text-green-700" : "text-pink-700"}`}>{passwordError.message}</p>
            </label>
            <input type="submit" className="w-full py-3 mt-8 bg-orange-500 hover:bg-orange-600 relative text-white shadow-md rounded-full" value="Daftar" />
