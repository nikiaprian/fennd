...
function ProfilePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await axios
        .get("http://35.93.77.214:9090/user/profile", {
          headers: {
    ...
    fetchBlogs("http://35.93.77.214:9090/blogs", token);
    fetchForums("http://35.93.77.214:9090/forums", token);
    fetchData();
  ...
  const handleSubmit = async (e) => {
    ...
    await axios
      .patch("http://35.93.77.214:9090/user/update-profile", data, {
        headers: {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
        },
      ...
      .then((res) => {
        const { username, photo } = res?.data?.data;
        setProfile({ photo: photo, username: username });
        Swal.fire("Berhasil!", "Anda Telah Berhasil Update Profile", "success").then((isConfirmed) => {
              ...
              <div className="rounded-md bg-white">
                <form>
                  <div className="flex flex-col gap-3 items-center px-5 py-4">
                    <img className="rounded-full w-32 h-32 object-cover border-2 border-orange-600" src={profile?.photo === null ? gambarUser : profile?.photo} alt="" />
                    <h4 className="text-lg font-medium mt-2">{profile.username}</h4>
                    {buttonUpdate ? (
                      <>
                        <button onClick={() => setButtonUpdate(false)} className="py-2 px-4  bg-orange-600 rounded-full">
                          X
                        </button>
                        <InputFile title="Image Profile Picture" input={setInputFile} type="profilUpdate" />
            ...
            <div className="flex flex-col md:p-12 md:flex-row mt-0 pt-0">
              <div className="w-11/12 md:w-7/12 mx-auto flex flex-col gap-4">
                <div className="text-center flex flex-row justify-center mb-6 ">
                  <button style={isForum ? activeStyle : null} className="mr-10 text-2xl hover:text-orange-500 active:text-orange-500 active:border-b-2 active:border-orange-400" onClick={() => handleClickForum()}>
                    ForumIn
                  </button>
                  <button style={!isForum ? activeStyle : null} className="text-2xl cursor-pointer hover:text-orange-500 active:text-orange-500 active:border-b-2 active:border-orange-400" onClick={() => handleClickBlog()}>
                    BlogIn
                  </button>
                </div>
                    ...
                    <div className="flex w-11/12 flex-col gap-4 mx-auto">
                      {blog &&
                        blog
                          .filter((data) => data?.user?.id === +idUser)
                          .map((data, index) => (
                            <CardBlog key={index} id={data.id} title={data.title} content={data.content} date={data.created_at.substring(0, 10)} img={data.photo} profileImg={data?.user?.photo} user={data.user.username} tags={data.tag} />
                          ))}
                    </div>
                  )}
                </div>
