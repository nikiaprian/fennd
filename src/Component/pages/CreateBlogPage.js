...
function CreateBlogPage() {
  ...
  const handleClick = async (e) => {
    ...
    await axios
      .post("http://35.93.77.214:9090/blogs/new", data, {
        headers: {
      ...
      <div className="w-screen h-screen overflow-x-hidden">
        ...
        <div className=" mt-40 w-10/12 md:w-8/12 mx-auto flex flex-col gap-4">
          ...
          <div className="my-10 flex flex-col gap-6 ">
            <InputJudul setInputJudul={setInputJudul} label="Judul Blog" placeholder="Masukkan judul Blog" />
            <InputTags tag={inputTag} setTags={setInputTag} />
            <InputFile input={setInputFile} type="createBlog" />
            <InputMarkdown setEditorState={setInputMarkdown} deskripsi="Deskripsi" mode="wysiwyg" placeholder="Tulis deskripsi anda disini" type="createBlog" />
