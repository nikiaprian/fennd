...
function LikeUnlikeAnswer(props) {
  ...
  const handleLike = async () => {
    if (state?.status_like) {
      await axios
        .delete(`http://35.93.77.214:9090/like/forum/comment/${state?.id}`, {
          headers: {
      ...
      await axios
        .post(
          `http://35.93.77.214:9090/like/forum/comment/${state?.id}`,
          {},
          {
            headers: {
              "Content-Type": "application/json",
    ...
    <>
      {state?.status_like ? (
        <div className="flex items-center gap-2">
          ...
          <p className="text-base wl-8 font-medium font-poppins">{state?.total_like}</p>
        </div>
      ) : (
        <div className="flex items-center gap-2">
