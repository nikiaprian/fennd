...
function DeleteAnswer(props) {
  ...
  const handleDelete = async () => {
    if (state?.userId === +idUser) {
      await axios
        .delete(`http://35.78.193.13:9090/commentsforum/${state?.id}`, {
          headers: {
  ...
  return (
    <>
      <div className="flex items-center gap-2">{state?.userId === +idUser ? <TrashIcon className="h-6 w-6 text-red-600 cursor-pointer" onClick={key && handleDelete} /> : null}</div>
    </>
  );
}
