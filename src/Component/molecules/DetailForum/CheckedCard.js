...
function CheckedCard(props) {
  ...
  const getApI = async (checked) => {
    ...
    await axios
      .patch(
        `http://35.93.77.214:9090/forum/comment/${props?.id}/selected-answer`,
        { is_answer: checked },
        {
          headers: {
  ...
  return (
    <>
      <div className="flex items-center gap-1">
        {checked ? (
          <CheckCircleIconSolid className="h-6 w-6 text-teal-700 cursor-pointer" onClick={handleChecked} />
        ) : (
          <CheckCircleIconSolid className={`h-6 w-6 text-gray-400 cursor-pointer ${forum?.user?.id === +idUser ? "block" : "hidden"}`} onClick={handleChecked} />
        )}
      </div>
    </>
  );
